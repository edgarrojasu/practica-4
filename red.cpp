...
void Red::mostrarRed() {
    ...
    // Mostrar tablas de costos de cada router
    for (auto& par : routers) {
...
void Red::calcularCaminos(int origen) {
    // Reiniciar todos los routers antes de Dijkstra
    for (auto& par : routers) {
        par.second->reinicio();
    }
    ...
    // Guardar los resultados en la tablaCostos del router origen
...
// Formato del archivo:
//   router <id>         -> agregar router (id es un numero: 0=A, 1=B, ...)
//   enlace <id1> <id2> <costo>  -> conectar dos routers
